 ...
 * C/C++ source code generated on : Mon May 18 20:22:16 2026
 *
...
void foc_controller_step(void)
{
  ...
  ialpha = 0.251327395F * err_d + foc_controller_DW.int_d;
  ibeta = 0.251327395F * err_q + foc_controller_DW.int_q;
  v_mag = sqrtf(ialpha * ialpha + ibeta * ibeta);
  if (v_mag > V_lim) {
    ialpha = ialpha * V_lim / v_mag;
    ibeta = ibeta * V_lim / v_mag;
  } else {
    foc_controller_DW.int_d += 0.16964601F * err_d;
    foc_controller_DW.int_q += 0.16964601F * err_q;
  }
